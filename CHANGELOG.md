...
**Incident, and the guard for it.** During the v48 browser test a fake sync code was put into the `localhost` test copy and not removed. The app syncs on page load, before a test can stub `fetch`, so later reloads created a junk row (`bw-000…`) in William's Supabase. His own row was never touched. Now `isLocalTestCopy()` makes every sync request from a local copy fail — FAILURE-MODES §9. With his OK, the junk row and an old `bw-TES…` test row (2026-08-08) were deleted the same day (migration `delete_test_sync_rows`, previewed first, his row excluded explicitly); his row was confirmed unchanged afterwards (5 food days, 50 gym days, same `updated_at`).
