...
- **Categories are stable ids** (`MEAL_CATEGORIES`: `breakfast`, `am-snack`, `lunch`, `pm-snack`, `post-workout`, `dinner`, `evening-snack`), **plus William's own** in `food.customCategories` (`[{id: 'c-…', label}]`, added from the meal-type dropdown's "＋ Add another…"; labels ≤ 30 chars; a name matching an existing type is reused, not duplicated). Always go through `allCategories()` / `categoryById()` — they cover both, and grouping, History, import and the inbox all use them. Store the id, never the label. Meals from before v39 have no category and are **never rewritten** — `mealCategory()` groups them at render time.
...
- **Meal editor layout (v50, William's design):** meal-type dropdown + time on one line (time auto-fills with `nowTimeLabel()` for a new meal *today*, blank on a past day, kept when editing), a running total (`mealTotalHtml`, updated live as numbers are typed), the big **🔎 Search food…** bar, then ✏️ Add manually / 📷 Photo (the photo box only appears after tapping Photo), items as cards (`mealItemCard`; tap to edit with labelled fields; `d.editing` is the open card), and name/notes under "More details". No empty starter row — search and Add manually add rows. A save blocked by a bad number opens that card. Saved-meal mode shows the name up top and no time/photo/details.
...
**The dated history now lives in `CHANGELOG.md`** (newest first). Docs are current through the v50 meal-editor commit (2026-09-25). Before writing new entries, run `git log --oneline -5` and compare against the top of `CHANGELOG.md` — anything newer is undocumented. Add the new entry to the top of `CHANGELOG.md`, and fold any rule that's still in force into **Current state** above.
